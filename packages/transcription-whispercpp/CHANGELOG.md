...
- Android prebuild shipped only CPU + Vulkan `.so` backends — `libggml-opencl.so` was missing despite the `whisper-cpp[opencl]` recipe being green and the `opencl` feature being declared in `vcpkg.json`. Root cause was that `CMakeLists.txt` never appended `"opencl"` to `VCPKG_MANIFEST_FEATURES` (only `"tests"` and `"vulkan"` were wired through), so vcpkg resolved the addon without the `opencl` feature and consequently without enabling `-DGGML_OPENCL=ON` on the `whisper-cpp` port. Added an `ENABLE_OPENCL` option (default `ON` on Android, `OFF` elsewhere — the feature is `platform: android` gated in `vcpkg.json` anyway) that appends `"opencl"` to `VCPKG_MANIFEST_FEATURES`, mirroring the `SD_OPENCL` pattern in `packages/diffusion-cpp`. [QVAC-18300]
