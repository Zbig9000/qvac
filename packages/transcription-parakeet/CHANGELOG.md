...
- Bumped the `parakeet-cpp` vcpkg pin to `2026-06-10` (registry baseline `1793830`), which refreshes the bundled `ggml-speech` to the current speech-branch tip `bec032cd`. The `parakeet-cpp` C++ tree is unchanged since the previous `128dae42` pin, so this only moves `ggml-speech`; prebuilds and the desktop RTF benchmark now build against the latest speech stack (QVAC-20614).
