...
- `vcpkg.json`: the bare `whisper-cpp` dependency is replaced with
  per-platform feature selections mirroring `transcription-whispercpp 0.9.0`:
...
`0.1.2` already pulled the Android GPU backends transitively because
...
`ensureBackendsLoadedAndroid()` (added in `0.1.1`):
